...
- 비자, 보험, 입국 규정, 여행 안전 정보 등 실질적이고 신뢰할 수 있는 내용을, 내외부 문서와 API에서 수집하고, LLM과 RAG로 실시간 응답


### ☁️ 필요성
- 해외 정보는 복잡하고 흩어져 있어 사용자 접근성이 낮음
- 대사관·정부 기관의 문서 기반 정보 수집 자동화 필요
- 단순 검색이 아닌 **의도 중심 질의응답 시스템** 필요

### ☁️ 목표
- 다국가 문서 기반 벡터 검색 & 응답 시스템 구축
- PDF 문서 자동 처리 및 QA 생성
- LLM 파인튜닝으로 응답 품질 개선
- FastAPI 기반 API 서비스 구축
...
| 항목 | 결과 |
|------|------|
| PDF 수집 및 임베딩 | 약 60개 문서, 3,200+ 청크 처리 |
| RAG 검색 | 평균 응답 문서 정확도 92% |
| QA 생성 | 자동 생성된 QA 쌍 1,000개 이상 |
| 파인튜닝 | Flan-T5 LoRA 기반 학습, 정답률 개선 |
| API | FastAPI 전 구간 테스트 완료 (Swagger/Postman) |
...
![Map](https://github.com/user-attachments/assets/7e2f2bec-47db-41d9-a176-367e64cf8bc7)
